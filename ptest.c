...
int main( int argc, char *argv[])
{
    ...
    double *col_val = NULL;
    int *col_ind = NULL;
    double start_llu, start_luu, end_llu, end_luu, start1, end1, start2, end2,
    ...
    while (iter < 100)
    {
        ...
        fprintf(result_file, "%lf \t%lf ", t_solve, (end1 - start1) );
#ifdef MY_PRINT_BASIS
        ...
        //----------------------------------------------------------------------
        start_luu = SUITESPARSE_TIME;

        OK(SPEX_update_lu_colrep(F1, vk, k, option));

        end_luu = SUITESPARSE_TIME;
        ...
        if (iter > 0)
        {
            ...
            start_luu3 = SUITESPARSE_TIME;

            OK(SPEX_update_lu_colrep(F2, vk, k, option));

            end_luu3 = SUITESPARSE_TIME;
...
//        if (info == SPEX_OK) {printf("matrix is not singular!\n");}
        ...
        //----------------------------------------------------------------------
        // Timing stats
        double t_llu = (end_llu-start_llu) ;
        double t_luu3 = (end_luu3 - start_luu3) ;
        double t_luu = (end_luu - start_luu) ;
        if (iter > 0)
        {
            t1 += t_llu;
            t2 += t_luu;
